...
def main():
    ...
    # Inverse reconstruction error (VALID here)
    inv_error_mlp = torch.mean((x - x_rec_mlp) ** 2).item()
    inv_error_bliq = torch.mean((x - x_rec_bliq) ** 2).item()

    print("\n=== Inverse Reconstruction Error ===")
    print(f"MLP     : {inv_error_mlp:.6f}")
    print(f"BLiqNet : {inv_error_bliq:.6f}")

    # Plots
    plot_forward(y, y_pred_mlp, "MLP Forward")
    plot_forward(y, y_pred_bliq, "BLiqNet Forward")

    plot_inverse(x, x_rec_mlp, "MLP Inverse")
    plot_inverse(x, x_rec_bliq, "BLiqNet Inverse")
