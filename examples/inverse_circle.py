...
def main():
    ...
    bliq.fit(x, y, epochs=800,lambda_rec=0.1,lambda_inv=0.1,lambda_latent=0.1)
    ...
    # Plots
    # ---------------------------
    plot_forward(y, y_pred_mlp, title="MLP Forward")
    plot_forward(y, y_pred_bliq, title="BLiqNet Forward")

    plot_inverse(x, x_rec_mlp, title="MLP Inverse (Collapse)")
    plot_inverse(x, x_rec_bliq, title="BLiqNet Inverse (Structured)")

    plot_manifold(x_rec_bliq, y)

    # NEW (important for paper)
